...
def train(x: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor,
    ...
    for epoch in range(args.epochs):
        ...
        #     model.eval()
        #     train_loss, train_acc, train_bca = ensemble_eval(model, criterion, train_loader)
        #     test_loss, test_acc, test_bca = ensemble_eval(model, criterion, test_loader)
    ...
    model.eval()
    _, test_acc, test_bca = ensemble_eval(model, criterion, test_loader)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='random self ensemble')
    ...
    # ========================savings=======================
    model_path = f'/mnt/data1/cxq/advbenchmark/bci_adv_defense_model/RSE/{args.dataset}/{args.model}/{args.setup}'
    if args.online:
        model_path = f'/mnt/data1/cxq/advbenchmark/bci_adv_defense_model/online_RSE/{args.dataset}/{args.model}/{args.setup}'


    log_path = f'/home/xqchen/bci_adv_defense-main_copy/log/RSE'
    if args.online:
        log_path = f'/home/xqchen/bci_adv_defense-main_copy/log/online_RSE'
    if not os.path.exists(log_path):
        os.makedirs(log_path)
    log_name = os.path.join(log_path, f'{args.setup}_{args.dataset}_{args.model}.log')
    
    
    excel_path = f'/home/xqchen/bci_adv_defense-main_copy/excel/RSE'
    if args.online:
        excel_path = f'/home/xqchen/bci_adv_defense-main_copy/excel/online_RSE'
