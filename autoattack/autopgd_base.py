...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if self.norm == 'Linf':
            t = 2 * torch.rand(x.shape).to(self.device).detach() - 1 #随机噪声
            cha_std = x.std(axis=-1)[:,:,:,None].detach()
            x_adv = x + self.eps * cha_std * torch.ones_like(x
                ).detach() * self.normalize(t)#修改为通道方差参考
        elif self.norm == 'L2':
            t = torch.randn(x.shape).to(self.device).detach()
            cha_std = torch.norm(x.detach(), p=2, dim=-1, keepdim=True)
        ...
        alpha = 2. if self.norm in ['Linf', 'L2'] else 1. if self.norm in ['L1'] else 2e-2#这路的alpha是啥
        step_size = alpha * self.eps * cha_std * torch.ones([x.shape[0], *(
            [1] * self.ndims)]).to(self.device).detach()#修改为通道方差参考
        '''
        step_size.shape
        torch.Size([n_batch, 1, 1, 1])
        '''
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.min(torch.max(x_adv_1,
                        x - self.eps * cha_std), x + self.eps * cha_std)
                    ...
                    x_adv_1 = x + self.normalize(x_adv_1 - x
                        ) * torch.min(self.eps * cha_std * torch.ones_like(x).detach(),
