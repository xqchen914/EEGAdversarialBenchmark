...
from utils.pytorch_utils import bca_score, init_weights, print_args, seed, adjust_learning_rate, weight_for_balanced_classes, CenterLoss
from defense_lib import LabelSmoothLoss
...
def train(x: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor,
    ...
    if not args.ls:
        ...
        criterion = LabelSmoothLoss(n_class=n_class, alpha=0.1).to(args.device)
    criterion_cent = CenterLoss(num_classes=n_class, feat_dim=n_class, device=args.device)
    ...
    for epoch in range(args.epochs):
        ...
        for step, (batch_x, batch_y) in enumerate(train_loader):
            ...
            batch_adv_x = attack_lib.PGD_batch_cha(model,
                                                ...
                                                alpha=args.AT_eps/5,
                                                steps=10)
            # maximize shift inconsistency
            batch_adv_x = attack_lib.maximize_shift_inconsistency(model,
                                                                batch_adv_x,
                                                                batch_x,
                                                                batch_y,
                                                                criterion_cent,
            ...
            loss_cal = criterion(adv_logits, batch_y)
            loss_scr = criterion_cent(adv_logits - logits, batch_y)

            loss = loss_cal + args.alpha * loss_scr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ls', type=bool, default=True)
    ...
    # ========================savings=======================
    model_path = f'/mnt/data1/cxq/advbenchmark/bci_adv_defense_model/SCR_{args.AT_eps}/{args.dataset}/{args.model}/{args.setup}'
    if args.online:
        model_path = f'/mnt/data1/cxq/advbenchmark/bci_adv_defense_model/online_SCR_{args.AT_eps}/{args.dataset}/{args.model}/{args.setup}'

    log_path = f'/home/xqchen/bci_adv_defense-main_copy/log/SCR_{args.AT_eps}'
    if args.online:
        log_path = f'/home/xqchen/bci_adv_defense-main_copy/log/online_SCR_{args.AT_eps}'
    if not os.path.exists(log_path):
        os.makedirs(log_path)
    log_name = os.path.join(log_path, f'{args.setup}_{args.dataset}_{args.model}.log')
    
    excel_path = f'/home/xqchen/bci_adv_defense-main_copy/excel/SCR_{args.AT_eps}'
    if args.online:
        excel_path = f'/home/xqchen/bci_adv_defense-main_copy/excel/online_SCR_{args.AT_eps}'
